...
@Extension(
        ...
        description = "Pressure Source gets events from pressure sensor of android device. " +
                "This will output a key value map with pressure (pressure in mbar), sensor" +
        ...
        parameters = {
                @Parameter(
                        ...
                        description = "polling.interval is the time between two events in" +
                                " milliseconds. If a polling interval is specified events are" +
                                " generated only at that frequency even if the sensor" +
                                " value changes.",
                        defaultValue = "0L",
                        optional = true,
                        type = {DataType.LONG}
        ...
        examples = {
                @Example(
                        syntax = "@source(type = 'android-pressure' ,@map(type='keyvalue'))\n" +
                                "define stream pressureStream(sensor string, pressure float," +
                                " accuracy int)",
