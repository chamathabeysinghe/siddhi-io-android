...
@Extension(
        ...
        namespace = "source",
        description = "Gyroscope Source gets events from gyroscope sensor of android device. " +
                "The events " +
                "are related to device’s rate of rotation. This will output a key value map" +
                "with rotationX (rotation rate vector component in X direction), rotationY(in Y " +
                "diirection),rotationZ (in Z direction) sensor (sensor name), accuracy(could be " +
                "HIGH (3) ,MEDIUM (2) or LOW (1) ) and timestamp(time at which event arrives) as" +
                " keys",
        parameters = {
                @Parameter(
        ...
        examples = {
                @Example(
                        syntax = "@source(type = 'android-gyroscope' ,@map(type='keyvalue'))\n" +
                                "define stream gyroscopeStream(sensor string, rotationX float, " +
                                "accuracy int)",
