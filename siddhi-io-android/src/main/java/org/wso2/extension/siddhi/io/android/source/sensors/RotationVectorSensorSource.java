...
@Extension(
        ...
        namespace = "source",
        description = "Rotation vector Source gets events from rotation vector sensor of android " +
                "device. The events are related to device’s rate of rotation. " +
                "This will output a key value map with rotationX " +
                "(rotation vector component in X direction), rotationY(in Y diirection)," +
                "rotationZ (in Z direction), valueScalar (scalar value) sensor (sensor name), " +
                "accuracy(could be HIGH " +
                "(3) ,MEDIUM (2) or LOW (1) ) and timestamp(time at which event arrives) as keys",
        parameters = {
                @Parameter(
        ...
        examples = {
                @Example(
                        syntax = "@source(type = 'android-rotation' ,@map(type='keyvalue'))\n" +
                                "define stream rotaionStream(sensor string, rotationX float, " +
                                "accuracy int)",
                        description = "This will consume events from Rotation vector sensor " +
                                "transport when the sensor value is changed.\n"
                ),
                @Example(
                        syntax = "@source(type = 'android-rotation ' ,polling.interval = 100," +
                                "@map(type='keyvalue'))\n" +
                                "define stream rotationStream(sensor string, rotationX float, " +
                                "accuracy int)",
                        description = "This will consume events from Rotation vector sensor " +
                                "transport " +
...
public class RotationVectorSensorSource extends AbstractSensorSource {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (this.pollingInterval == 0L && (this.latestInput == null
                ...
                || (float) this.latestInput.get("valueScalar") !=
                (float) output.get("valueScalar"))) {
