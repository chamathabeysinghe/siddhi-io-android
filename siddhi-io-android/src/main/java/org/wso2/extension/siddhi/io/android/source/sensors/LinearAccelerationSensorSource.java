...
@Extension(
        ...
        namespace = "source",
        description = "Linear Accelerometer Source gets events from Linear accelerometer sensor " +
                "of android device." +
                " This will output a key value map with accelerationX ( linear acceleration " +
                "vector component in X direction), accelerationY(in Y diirection),accelerationZ " +
                "(in Z direction) sensor (sensor name), accuracy(could be HIGH (3) ,MEDIUM (2) " +
                "or LOW (1) ) and timestamp(time at which event arrives) as keys",
        parameters = {
                @Parameter(
        ...
        examples = {
                @Example(
                        syntax = "@source(type = 'android-linear-accelerometer' ," +
                                "@map(type='keyvalue'))\n" +
                                "define stream accelerometerStream(sensor string, " +
                                "accelerometerX float, accuracy int)",
                        description = "This will consume events from Linear Accelerometer " +
                                "sensor transport " +
    ...
    public void init(SourceEventListener sourceEventListener, OptionHolder optionHolder,
        ...
        if (sensor == null) {
            throw new SiddhiAppCreationException("Linear Accelerometer Sensor is not supported " +
                    "in the device. Stream " + sourceEventListener.getStreamDefinition().getId() +
